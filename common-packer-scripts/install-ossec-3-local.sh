...
echo 'Installing OSSEC (local) v3.0.0...'

sudo yum install -y inotify bind-utils pcre2-devel zlib-devel libevent-devel systemd-devel
...
wget -O /tmp/ossec-hids-3.0.0.tar.gz  http://192.168.1.4/source-code/ossec-hids-3.0.0.tar.gz
cd /tmp     # BEWARE - a directory change !
sudo tar xfvz ossec-hids-3.0.0.tar.gz
cd ossec-hids-3.0.0
...
echo "Finished setup.sh OK for provisioning OSSEC (local) v3.0.0 on this node"
echo
